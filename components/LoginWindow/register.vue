  ...
  <div class="modalWindow" :class="{'open': open}">
    <div class="modalWindow__wrapper">
      ...
      <form action="#" class="form modalWindow__form" @submit.prevent="register">
        ...
        <div class="form__group">
          <input type="password" required class="form__input" :class="{'form__input--error': ($v.repeatPassword.$dirty && $v.repeatPassword.$error)}" v-model="$v.repeatPassword.$model" placeholder="Пароль">
          <small class="form__error" v-show="$v.repeatPassword.$error">Пароли не схожи</small>
        </div>
        <div class="form__group form-group__checkbox">
          <label for="offers">
            <input type="checkbox" id="offers" v-model="offers">
            <span class="checkbox"></span>
            <p>Хочу получать выгодные предложения от магазина</p>
          </label>
        </div>
        <div class="form__group form-group__checkbox">
          <label for="policy" :class="{'error': $v.policy.$dirty && $v.policy.$error}">
            <input type="checkbox" id="policy" v-model="$v.policy.$model">
            <span class="checkbox"></span>
            <p>Принимаю условия <a href="#">Пользовательского соглашения, Политики конфиденциальности</a></p>
          </label>
        </div>
        <button class="form__submit login-submit">Зарегистрироваться</button>
      </form>
    </div>
  </div>
</template>
